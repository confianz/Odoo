...
class AuthorizenetApi(models.Model):
    ...
    environment = fields.Selection([('livemode', 'Live mode'), ('testmode_server', 'Test mode (in production server)'), ('testmode', 'Test mode') ], 'Environment', required=True, default='testmode')
    ...
    def create_payment_profile(self, profile_id, partner, card_no, cvv, expiry_date):
        ...
        auth_data = """<?xml version="1.0" encoding="utf-8"?> 
            ...
            """% (auth_header, profile_id, partner_name, company, address, self.check_string(partner.city), state or '', partner.zip or '', country or '', partner.phone or '',
    ...
    def refund_payment_aim(self, transaction_id, amount, invoice):
        ...
        success, response, header, code = self.execute_request(auth_datas, url=url, headers={'Content-Type': 'text/xml'})
    ...
    def authorize_capture_transaction_aim(self, amount, card, cvv, expiry, invoice):
        ...
        first_name = self.check_string(customer_details and customer_details.partner_id and customer_details.partner_id.name)
        street = False
        city = False
        country = False
        state = False
        zip = False
        email = False
        if customer_details and customer_details.id:
            street = self.check_string(customer_details.partner_id.street)
            city = self.check_string(customer_details.partner_id.city)
            country = customer_details.partner_id.country_id and customer_details.partner_id.country_id.name
            state = customer_details.partner_id.state_id and customer_details.partner_id.state_id.name
            zip = customer_details.partner_id.zip
        ...
        auth_data = """<?xml version="1.0" encoding="utf-8"?>
                    ...
                    """ % (auth_header, round(float(amount), 2), card, expiry, cvv, customer_details.number, id, email, first_name, street, city, state, zip, country)
        success, response, header, code = self.execute_request(auth_data, url=url, headers={'Content-Type': 'text/xml'})
    ...
    @api.model
    def authorize_capture_cheque_transaction_aim(self, amount, invoice, account_name, routing_number, account_number, bank_name, eCheque_type, account_type):
        """
        ...
        zip = False
        email = False
        if invoice_details and invoice_details.partner_id:
            street = self.check_string(invoice_details.partner_id.street)
            city = self.check_string(invoice_details.partner_id.city)
            country = (invoice_details.partner_id.country_id and invoice_details.partner_id.country_id.name) or ''
            state = (invoice_details.partner_id.state_id and invoice_details.partner_id.state_id.name) or ''
            zip_code = invoice_details.partner_id.zip
            email = invoice_details.partner_id.email
            company_name = (invoice_details.partner_id.parent_id and invoice_details.partner_id.parent_id.name) or ''
#            email = email and email.split(',')
#            email = email and str(email[0])
        auth_header = self._get_auth_header()
        url = self._get_url(company_id=self.company_id)
        ...
        auth_header, round(float(amount), 2), account_type, routing_number, account_number, account_name, eCheque_type, bank_name, invoice_details.number, invoice_details.number,
        id,invoice_details.number, round(float(amount), 2), invoice_details.partner_id.id, email,
        first_name, company_name, first_name,
