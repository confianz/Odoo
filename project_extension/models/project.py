...
class ProjectProject(models.Model):
    ...
    project_code = fields.Char(string="ProjectCode", required='True')
    _sql_constraints = [
    ...
    def _compute_invoice_count(self):
        for project in self:
            ...
            project.invoice_count = Invoice.search_count([('project_id', '=', project.id), ('type', '=', 'out_invoice')])
    ...
    def action_send_proposal_to_customer(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('project_extension', 'email_template_proj_proposal_customer')[1]
        except ValueError:
            template_id = False
        try:
        ...
        ctx = {
            ...
            'default_attachment_ids': [(4, attachment.id) for attachment in self.user_approved_proposal_id.attachment_ids],
#            'custom_layout': "mail.mail_notification_paynow",  ## Use custom_layout iff you want a link back to record in email
    ...
    def create_timesheet_invoice(self):
        ...
        """
        projects = self.search([('project_type', '=', 'time_sheet'), ('active', '=', True), ('next_billing_date', '=', fields.Date.today()), ('state', '=', 'proposal_accepted')])
    ...
    def create_fte_invoice(self):
        ...
        """
        projects = self.search([('project_type', '=', 'fte'), ('active', '=', True), ('next_billing_date', '=', fields.Date.today()), ('state', '=', 'proposal_accepted')])
        customer_ids = projects.mapped(lambda partner: partner.partner_id)
        for customer in customer_ids:
            for project in projects.filtered(lambda project: project.partner_id == customer):
                journal_account_id = project.get_default_sale_jrnl_acc().id
    ...
    def compute_description(self, res_id=None):
        ...
        """
        for project in self:
            product_desc = project.product_id.description_sale if (project.product_id and project.product_id.description_sale) else (project.product_id.name or '')
    ...
    def send_invoice_alerts(self, invoice_id=False, invoice_alert_ids=False):
        ...
        ctx.update({
            ...
            'custom_layout':"mail.mail_notification_paynow",  # used to decorate mail template, and create back link to record
            'force_email':True,
        })
...
#                    template.with_context(ctx).send_mail(invoice_id.id, force_send=True)
        ...
        wizard_send = self.env['account.invoice.send'].with_context(ctx).create({
                 ...
                 'composer_id': self.env['mail.compose.message'].with_context(ctx).create({'composition_mode': 'comment'}).id,
