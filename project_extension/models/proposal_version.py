...
class ProposalVersion(models.Model):
    ...
    def check_if_already_done_by_customer(self):
        ...
        """
        for proposal in self:
            if proposal.customer_done == True  or proposal.state in ['rejected']:
                return True
            else:
                return False
    ...
    def get_proposal(self,proposal):
        ...
        if self.env.user.has_group('group_user'):
            WEB_URL = '%s/proposal/version/%s' % (web_root_url, proposal.id)
            return WEB_URL
        elif self.env.user.has_group('group_portal'):
